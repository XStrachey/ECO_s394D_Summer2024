...
Let $Y_t=\begin{cases}0,\text{ if }x<t\\t,\text{ if }x\geq t\end{cases}$
...
$0\times P(X<t)+t\times P(X\geq t)\leq E(X)$

So,

$t\times P(X\geq t)\leq E(X)$
...
Given Markov's inequality, then:
...
It says that the mean of a large sample is close to the expected value of the distribution from which the sample is drawn:
...
$\text{Variability of an average}=\frac{\text{Variability of a single data point}}{\sqrt{\text{Sample size}}}$

$\text{Standard error of the mean}=\frac{\sigma}{\sqrt{N}}$

# Central Limit Theorem

Suppose that $X_1,...,X_N$ are IID random variables with mean $\mu$ and variance $\sigma^2$.

Let $\overline X_N$ be the sample mean.

Now define $Z_N$ as the z-score of the sample mean:

$Z_n=\frac{\overline X_n-\mu}{sd(\overline X_n)}=\frac{\overline X_n-\mu}{\sqrt{var(\overline X_n)}}=\frac{\overline X_n-\mu}{\sqrt{\frac{\sigma^2}{N}}}=\frac{\sqrt{N}(\overline X_n-\mu)}{\sigma}$

So, for all $n\in[1,N]$, there is:

$Z_n=\frac{\sqrt{N}(\overline X_n-\mu)}{\sigma}\sim N(0,1)$

# de Moivre's equation and CLT

$\theta\in\hat\theta±z\cdot se(\hat\theta)$
...
# Comparing two means
do(10000)*diffmean(age~sex, data=resample(titanic))
# Comparing two proportions
do(10000)*diffprop(age~sex, data=resample(titanic))
# Regression model
