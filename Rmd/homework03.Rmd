...
ggplot() +
  ...
  labs(x = "x", y = expression(F[X](x)),
       caption = "<b>Figure 1. The cumulative distribution function, or CDF, of a discrete random variable X is defined as:<br>X=2 with probability 0.1<br>X=3 with probability 0.1<br>X=8 with probability 0.8<br></b>") +
...
$P(X^2\leq a)=P(X\leq\sqrt a)=\begin{cases}0,\text{ for }a<0\\\sqrt a,\text{ for }0\leq a\leq1\\1,\text{ for }a>1\end{cases}$

### iii. From (ii), find the PDF of the random variable $Y=X^2$

From (ii), we have the CDF of $Y=X^2$:

$F_Y(y)=P(Y\leq y)=P(X^2\leq y)=P(X\leq\sqrt y)=\begin{cases}0,\text{ for }y<0\\\sqrt y,\text{ for }0\leq y\leq1\\1,\text{ for }y>1\end{cases}$

therefore, it is clear that:

$\text{PDF}_Y=f_Y(y)=\frac{d}{dy}F_Y(y)=\begin{cases}0,\text{ for } x\leq0\\\frac{1}{2}y^{\frac{-1}{2}},\text{ for }0<x\leq1\\0,\text{ for }x>1\end{cases}$

### iv. Compute $E(Y)$ and $var(Y)$ directly from the PDF

Given (iii), it is clear that:

$E(Y)=\int_{-\infty}^0y\cdot f_Y(y)dy+\int_0^1y\cdot f_Y(y)dy+\int_1^{+\infty}y\cdot f_Y(y)dy=0+\int_0^1\frac{1}{2}y^{\frac{1}{2}}dy+0=\frac{1}{3}$

For $var(Y)$, given the formula $var(Y)=E(Y^2)-(E(Y))^2$, it is necessary to calculate $E(Y^2)$ firstly:

$E(Y^2)=\int_{-\infty}^0y^2\cdot f_Y(y)dy+\int_0^1y^2\cdot f_Y(y)dy+\int_1^{+\infty}y^2\cdot f_Y(y)dy=0+\int_0^1\frac{1}{2}y^{\frac{3}{2}}dy+0=\frac{1}{5}$
...
It is clear that $E(Z_i)=0$ and $var(Z_i)=1$.

Because the formula $var(Z_i)=E(Z_i^2)-(E(Z_i))^2$, we can find:
...
$\therefore f_U(u)=\begin{cases}1,\text{ for }0\leq x\leq1\\0,\text{ otherwise}\end{cases}$

$\therefore F_U(u)=\begin{cases}0,\text{ for }x<0,\\x,\text{ for }0\leq x\leq1\\1,\text{ for }x>1\end{cases}$
...
ggplot(simulations) +
  ...
  labs(x = "N", y = "Standard Deviation", shape = "Type",
       caption = "<b>Figure 2. The plot shows comparison of Monte Carlo simulated and theoretical standard deviations.</b> <br>Through this figure, the Monte Carlo standard deviation of the simulation, which simulated 1000 realiza-<br>tions, agrees, at least approximately, with the theoretical standard deviation computed from the result in (A). <br>Besides, there is a pattern that the standard deviation decreases by the N increases, which demonstrates <br>that as the sample size increases, the mean of the observed values will gradually converge to the expected <br>probability. This is a verification of the Law of Large Numbers and the Central Limit Theorem.") +
  theme(plot.caption = element_text(hjust = 0)) + 
  theme(plot.caption = element_markdown())
```
...
$f_{Y_N}(y)=\frac{d}{dy}(1-e^{-\lambda y})^N=\lambda N(1-e^{-\lambda y})^{N-1}$
