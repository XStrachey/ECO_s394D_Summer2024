...
ggplot(capmetro_UT) +
  ...
  labs(x = "Hour of day (hours)", y = "Average boarding",
       caption = "<b>Figure 1. The average boarding of Austin’s own Capital Metro bus network, faceted by day of the <br>week, is based on data from September to November 2018.</b> Peak hours on weekdays are generally <br>consistent, with the peak period from Monday to Thursday being roughly from 2 PM to 7 PM. On Fridays, <br>the peak hours start and end about an hour earlier. Meanwhile, the passenger average boarding distribution <br>on weekends shows significant differences compared to weekdays. Overall passenger average boarding is <br>lower on weekends, and there is no distinct single peak in passenger volume. What's more, Labor Day may <br>have resulted in lower average boardings on Mondays in September. Meanwhile, Thanksgiving Day may <br>have lowered the average boardings on Wednesdays, Thursdays, and Fridays in November.") +
...
ggplot(capmetro_UT) +
  ...
  labs(x = "Temperature (℉)", y = "Boarding", color = "weekend status",
...
kable(top10,
      caption = "The top 10 most popular songs since 1958, as measured by the total number of weeks that a song spent on the Billboard Top 100. The data contains every song to appear on the weekly Billboard Top 100 chart since 1958, up through the middle of 2021.")
```
...
Based on the linear regression, a one-year increase in age is associated with an average decrease of 0.62 mL/minute in the creatinine clearance rate.
...
ggplot(dice2) +
  ...
  labs(x = "First dice number", y = "Second dice number", color = "Parity of Sum",
...
Since the attribute named ‘GP’ stands for ‘games played,’ it is clear that each team played 19 games as the home team and 19 games as the away team. Therefore, the total number of games is $(19\times20\times2)/2=380$. 380 games have been played with 1072 goals: 596 scored by home teams (average approximately 1.57 per game); 476 by away teams (average approximately 1.25 per game).
...
Given that the expected goals scored by each team are independent, and denoting the expected goals scored by the home team as $X$ and the expected goals scored by the away team as $Y$, the joint probability for a match is:  

$P(X=x,Y=y)=\frac{\lambda_X^x}{x!}e^{-\lambda_X}\times\frac{\lambda_Y^y}{y!}e^{-\lambda_Y}$
...
Following the approach outlined above, we can calculate the result of each match. For example, for the match between Liverpool (home) and Tottenham (away), to determine the probability of a 2-1 result, we multiply 27.0% by 33.8% to get approximately 9.1%, as demonstrated by the following calculation steps:

$\because$

$\lambda_{\text{excpect goals}_{\text{Liverpool}}}=\text{baseline}_{\text{home}}\times\text{AttackStrength}_\text{Liverpool}\times\text{DefenceWeakness}_\text{Tottenham}$

$\lambda_{\text{excpect goals}_{\text{Tottenham}}}=\text{baseline}_{\text{away}}\times\text{AttackStrength}_\text{Tottenham}\times\text{DefenceWeakness}_\text{Liverpool}$

$\therefore$

$\lambda_{\text{excpect goals}_{\text{Liverpool}}}=1.568421\times1.660448\times0.7276119\approx1.89$

$\lambda_{\text{excpect goals}_{\text{Tottenham}}}=1.252632\times1.25\times0.4104478\approx0.64$
...
Following the approach outlined above, we can calculate the result of each match. For example, for the match between Manchester City (home) and Arsenal (away), to determine the probability of a 2-1 result, we multiply 24.8% by 35.2% to get approximately 8.7%, as demonstrated by the following calculation steps:

$\because$

$\lambda_{\text{excpect goals}_{\text{Manchester City}}}=\text{baseline}_{\text{home}}\times\text{AttackStrength}_\text{Manchester City}\times\text{DefenceWeakness}_\text{Arsenal}$

$\lambda_{\text{excpect goals}_{\text{Arsenal}}}=\text{baseline}_{\text{away}}\times\text{AttackStrength}_\text{Arsenal}\times\text{DefenceWeakness}_\text{Manchester City}$

$\therefore$

$\lambda_{\text{excpect goals}_{\text{Manchester City}}}=1.568421\times1.772388\times0.9514925\approx2.65$

$\lambda_{\text{excpect goals}_{\text{Arsenal}}}=1.252632\times1.36194\times0.4291045\approx0.73$
